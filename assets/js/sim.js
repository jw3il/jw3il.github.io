...
const adjacency = new SquareMatrix();
const dist = new SquareMatrix();
const distNext = new SquareMatrix();
...
class GraphNode {
  ...
  static enter(selection) {
    return selection
      ...
      .attr("class", "node")
      .attr("r", 0)
      .call((e) => e.transition().duration(500).attr("r", GraphNode.radius))
...
class Packet {
  ...
  static exit(selection) {
    return selection
      ...
      .attr("r", 0)
      .each((p) => {
        if (p.arrived) deleteNode(p.node);
      })
      .remove();
  }
  ...
  if (nodes.length > 0) {
    ...
    if (nodes.length > 1 && Math.random() >= 0.7) {
      ...
      while (neighbors.length < 2) {
        var neighborCandidate = this.nodes[randRangeRound(0, nodes.length - 1)];
...
function step(time) {
  ...
  if (
    initialZoom.done &&
    ((nodes.length >= 10 && packets.length < maxNumPackets) ||
      (nodes.length > 2 && packets.length < 1))
  ) {
  ...
  if (nodes.length < targetNodes && buildUpElapsed >= 200) {
    ...
    sortedNodes = nodes
      .map((n) => ({
        n,
        sort: (Math.abs(n.x) * whr + Math.abs(n.y)) * Math.random(),
      }))
